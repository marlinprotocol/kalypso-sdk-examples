...
const createAskTest = async () => {
  ...
  // if no secret, just keep secretString = ""
  ...
  while (true) {
    try {
      ...
      if (proof && proof.proof_generated) {
        ...
        writeStringToFile("/home/ubuntu/noir_enclave_setup/hello_world/proofs/hello_world.proof",decoded[1]);
        console.log(decoded[1]);
        return; // Exit the loop if proof is successfully fetched
      }
      else if ((proof.proof=="") && !proof.proof_generated){
