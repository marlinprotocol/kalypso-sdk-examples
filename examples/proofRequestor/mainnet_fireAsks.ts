...
async function readFullData(): Promise<string> {
  try {
    const stream = await kalypso
      .MarketPlace()
      .MatchingEngineEnclaveConnector()
      .buildAttestation(false);
...
async function fireValidAsk(): Promise<never> {
  while (true) {
    ...
    const proofGenerationTimeInSec = new BigNumber(10000);
...
async function fireInvalidAsk(): Promise<never> {
  while (true) {
    ...
    await delay(invalidAskInterval);
  }
}
...
async function getExpiryTime(): Promise<BigNumber> {
  try {
    ...
    // Convert expiry time to a string and then to a BigNumber
    const expiryBigNumber: BigNumber = new BigNumber(expiryMs.toString());

    // Divide by 1000 to convert milliseconds to seconds
    const expiryInSeconds: BigNumber = expiryBigNumber.div(1000);
